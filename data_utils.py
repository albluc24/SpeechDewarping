...
import audio
speakers=["carla", "daria", "echo", "f", "massimo", "naso", "seria", "serio", "ulisse", "walt", "espeak"]
speaker2idx_libri100 ={s:n for n,s in enumerate(speakers)}
...
class TextMelLoader(torch.utils.data.Dataset):
    ...
    def get_mel_text_pair(self, audiopath_and_text):
        if self.hp.text_finetune:
            ...
            text = torch.IntTensor(_symbols_to_sequence(text, clean_symbols=False))
            ...
            if self.hp.concat_speaker_embedding and self.hp.num_speaker>1:
                return warped_mel, mel, torch.IntTensor([speaker2idx_libri100[audiopath.split('/')[-2]]]), audiopath
